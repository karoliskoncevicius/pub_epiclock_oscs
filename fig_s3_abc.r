...
dat <- read.csv("input/WBCminusNeu_clocksPC.csv", skip = 1)
...
pdf("fig_s3_abc.pdf", width = 18/2.54, height = 5/2.54, pointsize = 8)

par(mar = c(4,4,4,4), oma = c(0,2,0,0), mgp = c(2.5,0.5,0), cex.main = 1)
par(mfrow = c(1,3))

plotclock(dat[,"PC.Horvath.pan.tissue"], dat[,1], "PC Horvath Pan-tissue")
title("a", adj = 0, cex.main = 1.5)

plotclock(dat[,"PC.DNAmTL"], dat[,1], "PC DNAmTL")
title("b", adj = 0, cex.main = 1.5)

plotclock(dat[,"PC.GrimAge"], dat[,1], "PC GrimAge")
